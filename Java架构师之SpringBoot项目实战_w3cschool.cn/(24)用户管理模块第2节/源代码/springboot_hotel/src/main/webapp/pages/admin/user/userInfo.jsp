	        ...
	        <td>
	        	<c:choose>
	        		<c:when test="${map.use_status=='1' && map.is_admin=='0' }">
		       			<a href="<%=basePath%>/updateUseStatus.do?id=${map.id}&useStatus=0" class="tablelink">禁用</a>
	        		</c:when>
	        		<c:when test="${map.is_admin=='1' }">
		       			<a href="javascript:void(0)" class="tablelink">无</a>
	        		</c:when>
	        		<c:otherwise>
	        			<a href="<%=basePath%>/updateUseStatus.do?id=${map.id}&useStatus=1" class="tablelink">启用</a>
	        		</c:otherwise>
	        	</c:choose>
	        </td>
	      </tr>
